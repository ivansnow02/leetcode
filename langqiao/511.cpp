...
// 假设一篇英语文章的长度为 N
// 个单词。给定这篇待译文章，翻译软件需要去外存查找多少次词典？假设在翻译开始前，内存中没有任何单词。 输入描述 输入共 2
// 行。每行中两个数之间用一个空格隔开。

// 第一行为两个正整数 M 和 N，代表内存容量和文章的长度。

// 第二行为 N 个非负整数，按照文章的顺序，每个数（大小不超过
// 1000）代表一个英文单词。文章中两个单词是同一个单词，当且仅当它们对应的非负整数相同。
